...
export default function DashboardVendeurLayoutClient({ children, session }) {
  ...
  // Écoute l'event depuis la page messages pour masquer chrome mobile
  useEffect(() => {
  ...
  // Reset quand on quitte la page messages
  useEffect(() => {
  ...
  const menuItems = [
    {
      section: 'PRINCIPAL',
      items: [
        { href: '/dashboard/vendeur', label: 'Tableau de bord', shortLabel: 'Dashboard', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg> },
        { href: '/dashboard/vendeur/nouvelle-opportunite', label: 'Nouvelle opportunité', shortLabel: 'Déposer', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg> },
        { href: '/dashboard/vendeur/opportunites', label: 'Mes opportunités', shortLabel: 'Dossiers', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> },
      ]
    },
    {
      section: 'COMMUNICATION',
      items: [
        { href: '/dashboard/vendeur/demandes', label: 'Mes acheteurs potentiels', shortLabel: 'Acheteurs', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg> },
        { href: '/dashboard/vendeur/messages', label: 'Messages', shortLabel: 'Messages', badge: unreadCount, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg> },
      ]
    },
    {
      section: 'COMPTE',
      items: [
        { href: '/dashboard/vendeur/profil', label: 'Mon profil', shortLabel: 'Profil', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> },
        { href: '/dashboard/vendeur/mot-de-passe', label: 'Mot de passe', shortLabel: 'Sécurité', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg> },
        { href: '/dashboard/vendeur/supprimer', label: 'Supprimer le compte', shortLabel: 'Supprimer', danger: true, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M3 6h18"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg> },
      ]
    },
  ]
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F4F6F8' }}>
      <style>{`
        @media (max-width: 1024px) {
          .vend-desktop { display: none !important; }
          .vend-topbar { padding: 0 16px !important; }
          .vend-main { padding: 20px 16px 90px !important; }
        }
        @media (min-width: 1025px) {
          .vend-mobile { display: none !important; }
        }
        .vend-nav-item:hover { background: rgba(255,255,255,0.06) !important; }
        .vend-bottomnav::-webkit-scrollbar { display: none; }
        .vend-bottomnav { -ms-overflow-style: none; scrollbar-width: none; }
      `}</style>

      {/* SIDEBAR DESKTOP */}
      <aside className="vend-desktop" style={{
        ...
        {!collapsed && (
          <Link href="/dashboard/vendeur/profil" style={{ textDecoration: 'none' }}>
            <div className="vend-nav-item" style={{ padding: '14px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', transition: 'background 0.15s' }}>
              ...
              <div style={{ overflow: 'hidden', flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: '#6B7280', marginTop: 1 }}>Vendeur</div>
              </div>
            </div>
          </Link>
        )}
          ...
          {menuItems.map((group) => (
            <div key={group.section} style={{ marginBottom: 24 }}>
              ...
              {group.items.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className="vend-nav-item" title={collapsed ? item.label : ''}
        ...
        <div style={{ padding: collapsed ? '12px 8px' : '12px 12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <button onClick={() => signOut({ callbackUrl: '/login' })} className="vend-nav-item" title={collapsed ? 'Déconnexion' : ''}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, minHeight: '100vh' }}>
        ...
        {!isMsgChat && (
          <header className="vend-topbar" style={{ height: 64, background: '#fff', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', position: 'sticky', top: 0, zIndex: 40, flexShrink: 0 }}>
            <div className="vend-desktop" />
            <div className="vend-mobile" style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
              ...
              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <Link href="/dashboard/vendeur/nouvelle-opportunite" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#FF5A1F', color: '#141414', padding: '7px 12px', borderRadius: 8, fontSize: 12, fontWeight: 700, textDecoration: 'none' }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                  Déposer
                </Link>
              </div>
            </div>
            <div className="vend-desktop" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <Link href="/dashboard/vendeur/nouvelle-opportunite" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#FF5A1F', color: '#fff', padding: '8px 14px', borderRadius: 8, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                Déposer
              </Link>
              <Link href="/dashboard/vendeur/profil" title="Mon profil" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '4px 8px', borderRadius: 8, transition: 'background 0.15s' }}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: '#9CA3AF' }}>Vendeur</div>
                  </div>
                </div>
              </Link>
            </div>
        ...
        <main className="vend-main" style={{ flex: 1, padding: isMsgChat ? 0 : '36px 40px', minWidth: 0 }}>
          {children}
        </main>
      </div>
      ...
      {!isMsgChat && (
        <div className="vend-mobile" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#141414', borderTop: '1px solid rgba(255,255,255,0.07)', zIndex: 50, paddingBottom: 'env(safe-area-inset-bottom)' }}>
          <div className="vend-bottomnav" style={{ display: 'flex', overflowX: 'auto', height: 64, alignItems: 'center', padding: '0 4px' }}>
            {allItems.map(item => {
              const isActive = pathname === item.href
              return (
