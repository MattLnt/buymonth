...
export default function RegisterVendeurPage() {
  ...
  const [form, setForm] = useState({ email: '', password: '', role: 'VENDEUR', nomBureau: '', nomCEO: '', telephone: '', adresse: '' })
      ...
      <div className="register-left" style={{ width: '52%', background: 'linear-gradient(160deg, #141414 0%, #1F1F22 50%, #141414 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '48px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(255,90,31,0.15)', border: '1px solid rgba(255,90,31,0.3)', borderRadius: '20px', padding: '6px 14px', marginBottom: '28px' }}>
            ...
            <span style={{ fontSize: '12px', color: '#FF5A1F', fontWeight: 600 }}>Compte Vendeur</span>
          </div>
          <h2 style={{ fontSize: '38px', fontWeight: 700, color: 'white', lineHeight: 1.15, margin: '0 0 20px', letterSpacing: '-0.02em' }}>
            Cédez votre cabinet<br />en toute<br /><span style={{ color: '#FF5A1F' }}>confidentialité.</span>
          </h2>
          <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, margin: '0 0 40px', maxWidth: '340px' }}>
            Déposez votre dossier anonymisé et attendez que des acheteurs qualifiés vous contactent.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {[
              'Anonymat total — coordonnées jamais visibles',
              'Aucune commission à la vente',
              'Alertes email quand un acheteur débloque votre dossier',
              'Messagerie sécurisée avec les acheteurs',
            ].map((item, i) => (
        ...
        <div style={{ width: '100%', maxWidth: '520px' }}>
          ...
          <div className="register-mobile-hero" style={{ padding: '48px 28px 36px', position: 'relative', display: 'none' }}>
            ...
            <h1 style={{ fontSize: '28px', fontWeight: 700, color: '#fff', lineHeight: 1.2, margin: '0 0 8px', letterSpacing: '-0.02em' }}>
              {step === 1 ? <>Créer un compte<br /><span style={{ color: '#FF5A1F' }}>vendeur</span></> : <>Votre cabinet<br /><span style={{ color: '#FF5A1F' }}>comptable.</span></>}
            </h1>
            <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', margin: 0 }}>
              {step === 1 ? 'Gratuit · Pour toujours' : 'Ces informations restent strictement confidentielles.'}
            </p>
          </div>
          ...
          <div className="register-form-card" style={{ background: '#fff', padding: '32px 28px', borderRadius: 0 }}>
            ...
            <div style={{ marginBottom: '24px' }}>
              {step === 1 ? (
                <>
                  ...
                  <p style={{ fontSize: '13px', color: '#9ca3af', margin: 0 }}>Déposez vos dossiers en toute confidentialité</p>
                </>
              ) : (
                <>
                  <h2 style={{ fontSize: '22px', fontWeight: 700, color: '#141414', margin: '0 0 4px', letterSpacing: '-0.02em' }}>Votre cabinet</h2>
                  <p style={{ fontSize: '13px', color: '#9ca3af', margin: 0 }}>Ces informations restent strictement confidentielles</p>
                </>
              )}
            </div>
            ...
            {step === 1 && (
              <form onSubmit={handleStep1} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                ...
                <button type="submit" disabled={!allValid || loading}
                  style={{ width: '100%', background: !allValid || loading ? '#e5e7eb' : '#141414', color: !allValid || loading ? '#9ca3af' : 'white', fontWeight: 700, padding: '14px', borderRadius: '12px', border: 'none', cursor: !allValid || loading ? 'not-allowed' : 'pointer', fontSize: '15px', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  {loading
                    ? <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: 'spin 1s linear infinite' }}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>Vérification...</>
                    : 'Continuer →'}
                </button>
            ...
            {step === 2 && (
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                ...
                <button type="submit" disabled={loading || !form.nomBureau || !form.nomCEO}
                  style={{ width: '100%', background: loading || !form.nomBureau || !form.nomCEO ? '#e5e7eb' : '#141414', color: loading || !form.nomBureau || !form.nomCEO ? '#9ca3af' : 'white', fontWeight: 700, padding: '14px', borderRadius: '12px', border: 'none', cursor: loading || !form.nomBureau || !form.nomCEO ? 'not-allowed' : 'pointer', fontSize: '15px', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                  {loading
                    ? <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ animation: 'spin 1s linear infinite' }}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>Création en cours...</>
            ...
            <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <p style={{ fontSize: '13px', color: '#9ca3af', margin: 0 }}>
                Vous voulez acheter ?{' '}
                <Link href="/register/acheteur" style={{ color: '#141414', fontWeight: 700, textDecoration: 'none' }}>Créer un compte acheteur →</Link>
              </p>
