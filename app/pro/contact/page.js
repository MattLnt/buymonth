...
export default function ProContactPage() {
  const [form, setForm] = useState({ nom: "", email: "", sujet: "Demander une démo", message: "" });
      ...
      <div style={{ minHeight: "100vh", background: "#EEF1F6" }}>
        <style>{`
          ...
          @media (max-width: 768px) {
            ...
            .procontact-form-grid { grid-template-columns: 1fr !important; }
          }
        `}</style>
        ...
        <div>
          ...
          <div style={{ background: "linear-gradient(150deg, #16324F 0%, #1D4267 100%)", padding: "72px 24px 60px", textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <p style={{ fontSize: 11, fontWeight: 700, color: "#7CB8A8", letterSpacing: "0.1em", margin: "0 0 14px", position: "relative" }}>DÉMO</p>
            <h1 style={{ fontSize: 42, fontWeight: 700, color: "#fff", margin: "0 0 14px", letterSpacing: "-0.025em", position: "relative" }}>Réservez votre démo.</h1>
            <p style={{ fontSize: 15.5, color: "rgba(255,255,255,0.6)", margin: "0 auto", maxWidth: 460, lineHeight: 1.7, position: "relative" }}>
              Laissez-nous vos coordonnées et quelques mots sur votre activité : nous vous recontactons pour organiser une démonstration de BuyMonth adaptée à vos biens.
            </p>
          </div>

          <div className="procontact-layout" style={{ maxWidth: 980, margin: "0 auto", padding: "64px 24px 80px", display: "grid", gridTemplateColumns: "1fr 320px", gap: 40, alignItems: "start" }}>

            {/* Formulaire */}
            <div style={{ background: "#fff", border: "1px solid #EEF2F7", borderRadius: 18, padding: 32 }}>
              <h2 style={{ fontSize: 19, fontWeight: 700, color: "#193B5E", margin: "0 0 24px" }}>Demander une démo</h2>
              ...
              {sent ? (
                <div style={{ background: "rgba(36,158,124,0.08)", borderRadius: 16, border: "1px solid rgba(36,158,124,0.22)", padding: "44px 32px", textAlign: "center" }}>
                  ...
                  <h3 style={{ fontSize: 18, fontWeight: 700, color: "#1B7A5E", margin: "0 0 8px" }}>Demande envoyée !</h3>
                  <p style={{ fontSize: 14, color: "#249E7C", margin: "0 0 24px" }}>Nous vous recontactons rapidement pour organiser votre démo.</p>
                  <button onClick={() => { setSent(false); setForm({ nom: "", email: "", sujet: "Demander une démo", message: "" }); }}
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  <div className="procontact-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} rows={6} placeholder="Parlez-nous de votre activité, du nombre de biens, de vos disponibilités..." style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} />
                  </div>
            ...
            {/* Sidebar infos */}
            <div className="procontact-sidebar" style={{ position: "sticky", top: 88, display: "flex", flexDirection: "column", gap: 12 }}>
